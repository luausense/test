                        ...
                        if onScreen and rootPos.Z >= 0 then
                            ...
                            if Config.ESP.Skeleton then
                                for _, bone in ipairs(skeletonBones) do
                                    local idx, p1n, p2n = unpack(bone); local p1 = char:FindFirstChild(p1n); local p2 = char:FindFirstChild(p2n)
                                    if p1 and p2 then
                                        ...
                                        if line and vis1 and vis2 then pcall(function() line.Visible = true; line.From = Vector2.new(pos1.X, pos1.Y); line.To = Vector2.new(pos2.X, pos2.Y); line.Color = color end) end
                                    end
                                end
                            end
                        end
...
local function TPToMap()
    ...
    for _, mapName in ipairs(mapsList) do
        local map = Workspace:FindFirstChild(mapName) if map then local part = map:FindFirstChildWhichIsA("BasePart", true) if part then CachedRoot.CFrame = part.CFrame * CFrame.new(0, 5, 0) return end end
    end
end
